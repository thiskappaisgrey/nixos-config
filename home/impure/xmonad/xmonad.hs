...
myProjects =
  ...
  , Project
    ...
    , projectStartHook = 
      Just $ do
                           spawnOn wsMusic (myTerminal ++ "-t ytfzf")
    }
...
-- TODO rewrite this to have it resize better?
floatAndResize ::  W.RationalRect -> Window -> X ()
floatAndResize rect w = whenX (isClient w) $ do
                     modifyWindowSet (\ws ->  (W.float w rect ws))
                     refresh
                     -- return ()
                     -- float w
                -- (W.float w rect ws)
  ...
  --   io $ moveWindow d w 1280 0
  --   io $ resizeWindow d w width height
  --   float w
  ...
  -- Screenshots
    ...
    ^++^ subKeys
      ...
      -- SCRATCHPADS -- very useful feature
           ...
           -- I just use "do" notation.. idk how to use the combinators lol
           , ("M-S-e", addName "Toggle Dynamic Scratchpad" $ do
                 withFocused $ floatAndResize (W.RationalRect (1 / 3) 0 (2 / 3) (2 / 3))
  ...
  -- TODO I need to add a prompt for switching to windows quickly
    ...
    ^++^ subKeys
           ...
           (  [
              ...
              -- TODO I wonder if I can make this a script instead - so I can run it in the terminal
              , ( "M-C-;"
